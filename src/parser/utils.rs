use super::*;
use crate::{AstToken, Location, SymbolKind};

#[derive(Debug, PartialEq)]
pub enum ParseError {
	ExpectedToken { token: AstToken, expected: String, source: String, },
	LeftoverToken { token: AstToken },
	NoMatch,
}
...
impl ParseError {
	pub fn location(&self) -> Location {
		...
		match self {
			...
			LeftoverToken { token } => token.location(),
	...
	T: Parser<'a>
{
	...
	fn parse(&self, state: ParseState<'a>) -> ParseResult<'a, Self::Item> {
		let mut collection = Vec::new();
		let (mut state, mut last_parsed) = expect!(state, self.0, self.1.to_string(), "ParseLikeArgsRequired");
		loop {
			let (ns, comma) = optional!(state.advance(0), ParseSymbol(SymbolKind::Comma));
			if comma.is_none() {
