...
test_suite!(parse_type_declaration_name_with_params, "hello<A, B: string>", | state: ParseState | {
	let (state, _) = ParseTypeDeclarationName.parse(state).expect("Failed");
	assert!(matches!(
		state.peek().expect("Failed").kind(),
		TokenKind::Eof
...
test_suite!(parse_intersection_type, "hello & 123", | state: ParseState | {
	let (state, _) = ParseTypeReference.parse(state).expect("Failed");
	assert!(matches!(
		state.peek().expect("Failed").kind(),
		TokenKind::Eof
...
test_suite!(parse_parenthesized_type, "(hello & 123)", | state: ParseState | {
	let (state, _) = ParseTypeReference.parse(state).expect("Failed");
	assert!(matches!(
		state.peek().expect("Failed").kind(),
		TokenKind::Eof
...
test_suite!(parse_conditional_type, "T :: string ? 1 : 2", | state: ParseState | {
	let (state, _) = ParseTypeReference.parse(state).expect("Failed");
	assert!(matches!(
		state.peek().expect("Failed").kind(),
		TokenKind::Eof
...
test_suite!(parse_value_nested_types, "(hello | 123 & ...) :: true ? false : number", | state: ParseState | {
	let (state, _) = ParseTypeReference.parse(state).expect("Failed");
	assert!(matches!(
		state.peek().expect("Failed").kind(),
		TokenKind::Eof
