...
impl FunctionAssignName {
	pub fn to_prefix(&self) -> PrefixExpr {
		match self {
    		FunctionAssignName::AccessMethod(node) => PrefixExpr::Var(VarExpr::AccessMethod(node.clone())),
    		FunctionAssignName::AccessName(node) => PrefixExpr::Var(VarExpr::AccessName(node.clone())),
