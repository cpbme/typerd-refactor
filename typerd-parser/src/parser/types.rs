...
define_parser!(
	...
	|_, state: ParseState<'a>| {
		let (state, result) =
			ParseLikeArgsRequired(ParseTypeReference, "<type>".to_string()).parse(state)?;
...
define_parser!(
	...
	ast::TypeGenericParameter,
	|_, state: ParseState<'a>| {
		let (state, name) = ParseName.parse(state)?;
		let (mut state, colon) = optional!(state, ParseSymbol(SymbolType::Colon));
...
define_parser!(
	ParseTypeGenericParameters,
	ast::TypeGenericParameters,
	|_, state: ParseState<'a>| {
		let (state, open) = ParseSymbol(SymbolType::LessThan).parse(state)?;
		let (state, members) =
			ParseLikeArgsRequired(ParseTypeGenericParameter, "<type>".into()).parse(state)?;
		let (state, close) = expect!(
			state,
			ParseSymbol(SymbolType::GreaterThan),
			">",
			"ParseTypeGenericParameters"
		);
		Ok((
...
define_parser!(ParseLiteralType, ast::LiteralType, |_,
                                                    state: ParseState<
	'a,
>| parse_either!(state, {
...
define_parser!(
	...
	|_, state: ParseState<'a>| {
		...
		let (state, close) = ParseSymbol(SymbolType::CloseParen).parse(state)?;
		Ok((
			state,
			ast::ParenthesizedType {
...
define_parser!(ParseGenericType, ast::GenericType, |_,
                                                    state: ParseState<
	'a,
>| {
	let (state, name) = ParseName.parse(state)?;
	let (state, open_arrow) = ParseSymbol(SymbolType::LessThan).parse(state)?;
	let (state, args) = ParseTypeArguments.parse(state)?;
	let (state, close_arrow) = ParseSymbol(SymbolType::GreaterThan).parse(state)?;
	Ok((
		state,
		ast::GenericType {
			name,
...
define_parser!(
	ParseTypeDeclarationName,
	ast::TypeDeclarationName,
	|_, state: ParseState<'a>| {
		let (state, name) = ParseName.parse(state)?;
		let (state, params) = optional!(state, ParseTypeGenericParameters);
		Ok((state, ast::TypeDeclarationName { name, params }))
	}
);
...
define_parser!(
	...
	|_, state: ParseState<'a>| {
		...
		if let Ok((ns, double_colon)) = try_parse!(state, ParseSymbol(SymbolType::DoubleColon)) {
			let (ns, compared_type) = expect!(
				ns,
				ParseTypeReference,
				"<type>",
				"ParseConditionalType(Compared)"
			);
			let (ns, true_token) = ParseSymbol(SymbolType::QuestionMark).parse(ns)?;
			let (ns, true_type) = expect!(
				ns,
				ParseTypeReference,
				"<type>",
				"ParseConditionalType(TrueType)"
			);
			let (ns, else_token) = ParseSymbol(SymbolType::Colon).parse(ns)?;
			let (ns, else_type) = expect!(
				ns,
				ParseTypeReference,
				"<type>",
				"ParseConditionalType(ElseType)"
			);
			Ok((
				ns,
				ast::TypeReference::Conditional(ast::ConditionalType {
		...
		} else if let Ok((ns, operator)) = try_parse!(state, ParseSymbol(SymbolType::VerticalBar)) {
			let (ns, right) = expect!(ns, ParseTypeReference, "<type>", "ParseUnionType");
			Ok((
				ns,
				ast::TypeReference::Union(ast::UnionType {
					left: Box::new(base),
					operator,
					right: Box::new(right),
				}),
			))
		} else if let Ok((ns, operator)) = try_parse!(state, ParseSymbol(SymbolType::Ampersand)) {
			let (ns, right) = expect!(ns, ParseTypeReference, "<type>", "ParseIntersectionType");
			Ok((
				ns,
				ast::TypeReference::Intersection(ast::IntersectionType {
					left: Box::new(base),
					operator,
					right: Box::new(right),
				}),
			))
		} else if let Ok((ns, left)) = try_parse!(state, ParseSymbol(SymbolType::OpenBracket)) {
			let (ns, right) = expect!(
				ns,
			...
			Ok((
				ns,
				ast::TypeReference::Array(ast::ArrayType {
